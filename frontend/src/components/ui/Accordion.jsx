...
export default function Accordion({ title, subtitle, badge, defaultOpen = false, children, className }) {
  ...
  return (
    <div className={cn('overflow-hidden rounded-xl border border-line bg-fill', className)}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex w-full items-center gap-3 p-4 text-left transition hover:bg-fill"
      >
        <ChevronDown
      ...
      {open && (
        <div className="animate-fade-in border-t border-line p-4 pt-4">{children}</div>
      )}
    </div>
  )
}
